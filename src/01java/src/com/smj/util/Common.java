...
public class Common {
	...
	private static final String randStr =  getRandStr(64, false);
	...
	public static Map<String, String> sysDirectory   = new HashMap<String, String>();//系统字典
    ...
    /**
	 * URL加密与解密
	 * param String s:需加密与解密的字符串
	 * param int i: 加密 i=1，解密i=2
	 * 
	 * @author 苏皓
	 */
    public static String url(String s, int i){  
        try {  
        	return crypt(s, randStr, i);
        } catch (Exception e) {  
            return null;  
        }  
    }  
	 ...
	 *    查询树的子节点
	 * */
	public List FindCateSubRoot(int subRoot)
	{
		List schList = new ArrayList();
		int treeId = subRoot;
	 ...
	 * 查询树的主键ID
	 * */
	public List FindCateId(int classid,int parentid)
	{
	    List schList = new ArrayList();
	    Map  condition = new HashedMap();
	    condition.put("condition","eq");
		condition.put("field","classId");
		condition.put("value",classid);
		condition.put("condition","eq");
		condition.put("field","parentId");
		condition.put("value",parentid);
		schList.add(condition);
	 ...
	 * 查询KEY返回VALUE
	 * */
	public  static String SysDictionary(HttpServletRequest request,String key){
		Map<String, String> sysDictionary = new HashMap<String, String>();
		String  value = "";
		System.out.println("getkey" +   key);
		JavaCenterHome.setJchRoot(request);
		String path  = JavaCenterHome.jchRoot + "\\WEB-INF\\classpath\\propsmj.properties";
		try {
			PropertiesHelper  properties = new PropertiesHelper(path);
