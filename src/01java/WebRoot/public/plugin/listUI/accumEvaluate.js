...
(function($, window, document) {
	$.addListUI = function(t, p) {
		...
		var d = {
			...
			pageStat : '显示第[ {from} - {to} ]条,共累计[{total}]条评价',// 显示当前页和总页面的样式
		...
		var g = {
			addData	: function(data) {
				...
				$(totalNum).text('共' + data.totalCount + '条累计评价');
				...
				$.each(data.data, function(i, n) {
					...
					$(spanContent).addClass('listInfoLiSpanText');
					var strContent  = n.content + "|" + Date.parse(n.date);
					$(spanContent).text(strContent);
