...
(function($) {
	...
	$.addFlex = function(t, p) {
		...
		p = $.extend({
			...
			rp : 15, // 每页默认的结果数
			rpOptions : [10, 30, 50, 70, 100],// 可选择设定的每页结果数
			title : false,// 是否包含标题
			pagestat : '显示第{from}条到{to}条,共{total}条数据',// 显示当前页和总页面的样式
			procmsg : '正在处理,请稍候 ...',// 正在处理的提示信息
			query : '',// 搜索查询的条件
			qtype : '',// 搜索查询的类别
			nomsg : '找不到符合条件的数据!',// 无结果的提示信息
...
//							    cthch.addClass("noborder").click(function() {//add matychen 不要noborder，加入后ie8显示有问题。
		...
		if (p.usepager) {
			...
			if (p.useRp) {
				...
				$('.pDiv2', g.pDiv).prepend("<div class='pGroup'><select name='rp' style='width:50px;height:24px;margin:0;padding:0;'>"
