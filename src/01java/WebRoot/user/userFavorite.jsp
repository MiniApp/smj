		...
		<div class="normgraybox">
			...
			<tr class="add_tr" onmouseover="this.className='add_tr_on';" onmouseout="this.className='add_tr';">
				<td width="180px">
					...
					<c:if test="${favorite.store_state eq 'Y'}"><a href="${ctx}/userAction.do?method=dishList&id=${favorite.id}">${smj:disposeLongStr(10,favorite.store_name)}</a></c:if>
					<c:if test="${favorite.store_state eq 'N'}">${smj:disposeLongStr(10,favorite.store_name)}</c:if>
				</td>
				<td width="150px">
					...
					<img src="${ctx}${smj:rankPic(favorite.rank,'S')}" alt="" title="${favorite.rank}诚信积分" border="0">
				</td>
				<td>${smj:disposeLongStr(20,favorite.address)}</td>
				<td width="30px">
