...
<html xmlns="http://www.w3.org/1999/xhtml">
										...
										<c:when test="${!empty affl}">
											...
											<c:forEach items="${affl}" var="aff">
												<tr class=line>
													<td>
														<a
															href="${ctx}/userAction.do?method=affiche&type=mes&id=${aff.id}"
															target="_black">${aff.title}</a>
													</td>
													<td align="center">
														${aff.date}
													</td>
													<td align="center">
														【
														<a
															href="${ctx}/merchantAction.do?method=affL&st=del&id=${aff.id}">删除</a>】
													</td>
												</tr>
											</c:forEach>
											<tr>
												<td colspan="3">
													${sugu:htmlPages(p_no,p_num)}
												</td>
											</tr>
										</c:when>
										<c:otherwise>
	...
	function set_sign(){
		document.location.href="${ctx}/merchantAction.do?method=addAnnounce&action=show";
	}
</script>
	</body>
</html>